...
class SrcTestCase(unittest.TestCase):
	...
	def test_url_api_issues_POST(self):
		...
		res = self.client.post('/api/v1/issues/', data=json_data, content_type='application/json',
	...
	def test_url_api_issues_GET(self):
		...
		issue_db = issue_db[0]
		issues = self.client.get('/api/v1/issues/')
	...
	def test_url_api_issue_GET_one(self):
		...
		issue_db = models.Issue.objects.get_or_create(**self.issue_default)
		issue_db = issue_db[0]
		issue = self.client.get('/api/v1/issues/%s' % (issue_db.register))
	...
	def test_url_api_issue_DELETE(self):
		...
		issue = self.client.delete('/api/v1/issues/%s' % (register))
		not_found = self.client.get('/api/v1/issues/%s' % (register))
	...
	def test_url_api_issue_PUT(self):
		...
		issue = self.client.put('/api/v1/issues/', 
	...
	def test_url_api_comments_POST(self):
		...
		json_data_length = len(json_data)
		# to send post
		res = self.client.post('/api/v1/comments/', data=json_data, 
			content_type='application/json',
			content_length=json_data_length)
		# get modified issue
		issue = models.Issue.objects.get(register=register)
	...
	def test_url_api_comment_PUT_one(self):
		...
		json_data_length = len(json_data)	
		URL = ''.join(['/api/v1/comments/', str(comment.pk), '/'])	
		res = self.client.put(URL, data=json_data, content_length=json_data_length,
			content_type='application/json')	
